  ...
  //                      if(!HAPLOID)
    ...
    //                    else
       //                         str=node[1]->getID().substr(n1,n2-n1-2);
		...
		MATCH_FILE2 << ALL_SNPS.getSNP(snp_start).getPhysPos() << ' ';
		MATCH_FILE2 << ALL_SNPS.getSNP(snp_end).getPhysPos() << '\t';
		...
		MATCH_FILE2 << ( snp_end - snp_start + 1) << '\t';

		MATCH_FILE2 << setiosflags(ios::fixed) << setprecision(2) << distance;
